<!-- product-list-filter -->
<div class="product-list-filter">
  <!-- <div class="product-list-filter-box">
    <span>筛选条件：</span>
    <a href="javascript:;" class="product-list-filter-item">品牌：好爸爸</a>
    <a href="javascript:;" class="product-list-filter-item">适用范围：酒店专用</a>
    <a href="javascript:;">全部清空</a>
  </div> -->
  <dl>
    <dt>品牌：</dt>
    <dd>
      <a href="#">好爸爸</a>
      <a href="#">威王</a>
      <a href="#">六必治</a>
    </dd>
  </dl>
  <dl>
    <dt>适用范围：</dt>
    <dd>
      <a href="#">通用</a>
      <a href="#">餐具专用</a>
      <a href="#">酒店专用</a>
      <a href="#">果蔬专用</a>
      <a href="#">贴身衣物/婴儿衣物</a>
      <a href="#">棉麻织物</a>
      <a href="#">丝绸毛料</a>
    </dd>
  </dl>
  <dl>
    <dt>净含量：</dt>
    <dd>
      <a href="#">1L-2L</a>
      <a href="#">2L以上</a>
      <a href="#">100g以下</a>
      <a href="#">101g-179g</a>
      <a href="#">180g-299g</a>
      <a href="#">300g-499g</a>
      <a href="#">500g-999g</a>
      <a href="#">1000g-2000g</a>
      <a href="#">2000g以上</a>
    </dd>
  </dl>
  <dl>
    <dt>计价单位：</dt>
    <dd>
      <a href="#">瓶</a>
      <a href="#">袋</a>
      <a href="#">箱</a>
      <a href="#">促销组合装</a>
    </dd>
  </dl>
</div>

<!-- content below is for mobile -->
<!-- product-list-filter -->
<div class="product-list-filter">
  <dl>
    <dt>品牌：</dt>
    <dd>
      <a href="#">好爸爸</a>
      <a href="#">威王</a>
      <a href="#">六必治</a>
    </dd>
  </dl>
  <dl>
    <dt>适用范围：</dt>
    <dd>
      <a href="#">通用</a>
      <a href="#">餐具专用</a>
      <a href="#">酒店专用</a>
      <a href="#">果蔬专用</a>
      <a href="#">贴身衣物/婴儿衣物</a>
      <a href="#">棉麻织物</a>
      <a href="#">丝绸毛料</a>
    </dd>
  </dl>
  <dl>
    <dt>净含量：</dt>
    <dd>
      <a href="#">1L-2L</a>
      <a href="#">2L以上</a>
      <a href="#">00g以下</a>
      <a href="#">101g-179g</a>
      <a href="#">180g-299g</a>
      <a href="#">300g-499g</a>
      <a href="#">500g-999g</a>
      <a href="#">1000g-2000g</a>
      <a href="#">2000g以上</a>
    </dd>
  </dl>
  <dl>
    <dt>计价单位：</dt>
    <dd>
      <a href="#">瓶</a>
      <a href="#">袋</a>
      <a href="#">箱</a>
      <a href="#">促销组合装</a>
    </dd>
  </dl>
</div>

<div class="product-list-filter-mobile">
  <div class="product-list-filter-mobile-content">
    <dl>
      <dt>
        <span>品牌：</span>
        <a href="javascript:;">全部<i class="iconfont"></i>
        </a>
      </dt>
      <dd>
        <label>
          <input type="checkbox" name="brand" checked="checked">
          <span>好爸爸</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="checkbox" name="brand" checked="checked">
          <span>威王</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="checkbox" name="brand" >
          <span>六必治</span>
        </label>
      </dd>
    </dl>
    <dl>
      <dt>
        <span>适用范围：</span>
        <a href="javascript:;">全部<i class="iconfont"></i>
        </a>
      </dt>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>通用</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>餐具专用</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>酒店专用</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>果蔬专用</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>贴身衣物/婴儿衣物</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>棉麻织物</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range1" checked="checked">
          <span>丝绸毛料</span>
        </label>
      </dd>
    </dl>
    <dl>
      <dt>
        <span>净含量：</span>
        <a href="javascript:;">全部<i class="iconfont"></i>
        </a>
      </dt>
      <dd>
        <label>
          <input type="radio" name="range2" checked="checked">
          <span>1L-2L</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range2" checked="checked">
          <span>2L以上</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range2" checked="checked">
          <span>100g以下</span>
        </label>
      </dd>
    </dl>
    <dl>
      <dt>
        <span>计价单位：</span>
        <a href="javascript:;">全部<i class="iconfont"></i>
        </a>
      </dt>

      <dd>
        <label>
          <input type="radio" name="range3" checked="checked">
          <span>瓶</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range3" checked="checked">
          <span>袋</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range3" checked="checked">
          <span>箱</span>
        </label>
      </dd>
      <dd>
        <label>
          <input type="radio" name="range3" checked="checked">
          <span>促销组合装</span>
        </label>
      </dd>
    </dl>
  </div>

  <div class="product-list-filter-btn">
    <a href="javascript:;" class="button button-grey button-radius">取消</a>
    <input type="button" value="确定" name="" class="button button-blue button-radius">
  </div>
</div>

